...
class TestRegressorMulitple(unittest.TestCase):
    ...
    def test_multiple_regression(self):
        ...
        self.assertTrue(abs_diff.max() < 3e-5, msg=f'betas differ: max abs diff={abs_diff.max()}')
        ...
        self.assertTrue(p_delta.max() < 0.015, msg=f'pvalue differs: max p_delta={p_delta.max()}')
    ...
    def test_multiple_regression_big_small(self):
        ...
        for n in [50, 500000]:
            ...
            abs_diff = difference(lstsq_fit[0][:-1], regressor_fit.coef_[:-1])
            self.assertTrue(abs_diff.max() < 3e-4, msg=f'max abs_diff={abs_diff.max()} for n={n}')
